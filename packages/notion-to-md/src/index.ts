...
export class NotionToMarkdown {
   ...
   */
  public setTransformer(type: string, transformer: CustomTransformer): NotionToMarkdown {
   ...
   */
  public async pageToMarkdown(id: string, totalPage: number | null = null): Promise<MdBlock[]> {
  ...
  public async blockToMarkdown(block: ListBlockChildrenResponseResult) {
    if (typeof block !== 'object' || !('type' in block)) return '';

    let parsedData = '';
    const { type } = block;
    if (type in this.transformers && !!this.transformers[type]) return this.transformers[type](block);
      ...
      case 'table': {
        ...
        if (has_children) {
          ...
          let rowsPromise = tableRows?.map(async (row) => {
             ...
             */
            let cellStringPromise = cells.map(async (cell: any) => {
              // TODO:  // GengarNeko 2022/08/9
              // eslint-disable-next-line @typescript-eslint/consistent-type-assertions
              const block = { type: 'paragraph', paragraph: { rich_text: cell } } as ListBlockChildrenResponseResult;
              await this.blockToMarkdown(block);
            });
   ...
   */
  public annotatePlainText(text: string, annotations: Annotations): string {
    ...
    if (text !== '') {
      if (annotations.code) newText = md.inlineCode(text);
      if (annotations.bold) newText = md.bold(text);
      if (annotations.italic) newText = md.italic(text);
      if (annotations.strikethrough) newText = md.strikethrough(text);
      if (annotations.underline) newText = md.underline(text);
    }
